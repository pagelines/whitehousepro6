...
class plRevSlider extends PageLinesSection {
	...
	function section_opts(){
		...
		$options[] = array(
			...
			'opts'	=> array(
					...
					array(
	                    'key' => 'revslider_text_color',
	                    'type' => 'color',
	                    'label' => __('Text Color', 'pagelines'),
	                    'default' => '000'
	                ),
					array(
	                    'key' => 'revslider_link_color',
	                    'type' => 'color',
	                    'label' => __('Link Color', 'pagelines'),
	                    'default' => '0088cc'
	                ),
					array(
	                    'key' => 'revslider_link_hover_color',
	                    'type' => 'color',
		...
		$options[] = array(
			...
			'opts'	=> array(
				...
				array(
					'key'	=> 'subtext',
					'label'	=> __( 'Slide Subtext', 'pagelines' ),
					'type'			=> 'text'
				),
				array(
					'key'	=> 'link',
	...
	function section_head( ){
		...
		<style type="text/css">
			...
			<?php echo $this->prefix(); ?> .slider-content h2{
				color: <?php echo pl_hashify($this->opt('revslider_text_color')); ?>;
			}
			
			<?php echo $this->prefix(); ?> .slider-content a.slider-btn{
				color: <?php echo pl_hashify($this->opt('revslider_link_color')); ?>;
			}
			
			<?php echo $this->prefix(); ?> .slider-content a:hover.slider-btn{
				color: <?php echo pl_hashify($this->opt('revslider_link_hover_color')); ?>;
			}
		...
		if( is_array($slide_array) ){
			...
			foreach( $slide_array as $slide ){
				...
				if( $the_bg || $extra ){
					...
					$content = sprintf('<h2><span class="slider-text">%s</span><br /><span class="slider-subtext">%s</span></h2>', $the_text, $the_subtext);
			...
			<li data-transition="fade" data-slotamount="10"  >
					...
					<div class="caption fade fullscreenvideo"
						...
						data-easing="easeOutBack">
							<iframe width="100%" height="100%" src="//www.youtube.com/embed/LvqMCiFyEGk?wmode=opaque" frameborder="0" allowfullscreen></iframe>
					</div>
