...
class WHPFooter extends PageLinesSection {
	...
	function section_opts(){
		...
		$opts = array(
			array(
				'title' => __( 'Footer Configuration', 'pagelines' ),
				'type'	=> 'multi',
				'opts'	=> array(
						array(
							'key'	=> 'wh_footer_area',
			...
			array(
				'key'	=> 'wh_footer_description',
				'type'	=> 'textarea',
			
				'title'	=> 'Column Site Description',
				'label'		=>	'Column Site Description',
				'help'		=> "If you use the default display of the footer, this field is used as a description of your company. You may want to add your address or links.",
			)
		);
	...
	function change_markup( $params ){
		...
		$cols = ($this->opt('wh_footer_cols')) ? $this->opt('wh_footer_cols') : 3;
   ...
   function section_template() {
		...
		if($area){
			...
			printf ('<div class="pl-shadow-wrap fix"><ul class="columnizer row fix sidebar_widgets">%s</ul></div>', $this->get_default() );
		}
