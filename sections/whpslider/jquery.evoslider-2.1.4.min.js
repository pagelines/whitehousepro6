...
return this;},_loadImage:function(src,container,floatPos,isMainImage,index,success,fail){var img,evo=this;img=new Image();img.src=src;$(img).bind("load",function(){if(isMainImage){evo._rescaleImage(img,container,index)}
$(this).css({"display":"none","float":floatPos}).prependTo(container).fadeIn("slow",function(){if($.isFunction(success)){success();}});}).bind("error",function(){if($.isFunction(fail)){fail();}});if($.browser.msie){$(img).attr({"src":src});}
