	...
	function section_opts(){
		...
		$options[] = array(
			...
			'opts'	=> array(
				...
				array(
                    'key' 		=> 'ibox_background_color',
                    'type' 		=> 'color',
                    'label' 	=> __('Background Color', 'pagelines'),
                    'default' 	=> 'f9f9f9'
                ),
				array(
                    'key' 		=> 'ibox_titles_color',
                    'type' 		=> 'color',
                    'label' 	=> __('Titles Color', 'pagelines'),
                    'default' 	=> '0088cc'
                ),
				array(
                    'key' 		=> 'ibox_text_color',
		...
		$options[] = array(
			...
			'opts'	=> array(
				...
				array(
					'key'		=> 'subtitle',
					'label'		=> __( 'iBox Subtitle', 'pagelines' ),
					'type'		=> 'text'
				),
				array(
					'key'		=> 'text',
   	...
   	function section_head( ){
		...
		<style type="text/css">
			...
			<?php echo $this->prefix(); ?> .ibox{
				background-color: <?php echo pl_hashify($this->opt('ibox_background_color')); ?>;
			}
			
			<?php echo $this->prefix(); ?> .ibox h4{
				color: <?php echo pl_hashify($this->opt('ibox_titles_color')); ?>;
			}
			
			<?php echo $this->prefix(); ?> .ibox .ibox-desc{
				color: <?php echo pl_hashify($this->opt('ibox_text_color')); ?>;
			}
   ...
   function section_template( ) {
		...
		if( is_array($ibox_array) ){
			...
			foreach( $ibox_array as $ibox ){
				...
				$subtitle = pl_array_get( 'subtitle', $ibox );
				...
				$title = sprintf('<h4 data-sync="ibox_array_item%s_title">%s <br /><span class="subtitle">%s<span></h4>', $count, $title, $subtitle );
				...
				$output .= sprintf(
					'<div class="span%s ibox %s %s fix">
						<div class="ibox-media img">
							%s
							<span class="ibox-icon-border pl-animation pl-appear %s" style="%s">
								%s
							</span>
							%s
						</div>
